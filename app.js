...
/* Live public analytics from the approved roll archive, per campaign.
   ...
   Session count/date-range/chart come from the vault index's all_sessions
   (every session, number+date only), falling back to the curated sessions
...
function rcDie(v){if(typeof v==='string'){try{v=JSON.parse(v);}catch(e){return null;}}return Array.isArray(v)&&Number.isFinite(v[0])?v[0]:null;}
function rcUnavailable(id){return function(){var c=D.filter(function(x){return x.id===id;})[0];if(!c)return;c.tr='Unavailable';c.se='Unavailable';c.n20='—';c.n1='—';c.sd=[];c.rt=[];c.ac=[];c.chars=[];c.lr=null;c.liveError='Live roll data is currently unavailable.';};}
function rcHydrate(cfg){return Promise.all([rcRows(cfg.view,cfg.label),fetch(cfg.index+'?t='+Date.now()).then(function(r){if(!r.ok)throw new Error(cfg.label+' session index unavailable.');return r.json();})]).then(function(v){var rows=v[0],index=v[1],c=D.filter(function(x){return x.id===cfg.id;})[0],byDate={},chars={},types={},actions={},n20=0,n1=0,sessions=(index&&(index.all_sessions||index.sessions))||[];if(!Array.isArray(sessions)||!sessions.length)throw new Error(cfg.label+' session index is empty.');var known={};sessions.forEach(function(s){known[s.d]=1;});var kept=0;rows.forEach(function(r){var d=r.session_date;if(cfg.sessionRollsOnly&&(!d||!known[d]))return;kept++;if(!d)return;byDate[d]=(byDate[d]||0)+1;if(r.character){chars[r.character]=chars[r.character]||{n:r.character,r:0,faces:[]};chars[r.character].r++;if(/^1d20/.test(r.dice_notation||'')){var face=rcDie(r.individual_values);if(face!==null)chars[r.character].faces.push(face);}}var type=r.roll_type||'Other';types[type]=(types[type]||0)+1;var action=r.action||'Other';actions[action]=(actions[action]||0)+1;if(/^1d20/.test(r.dice_notation||'')){var die=rcDie(r.individual_values);if(die===20)n20++;if(die===1)n1++;}});c.tr=kept;c.se=sessions.length;c.n20=n20;c.n1=n1;c.fr=sessions[0].d;c.lr=sessions[sessions.length-1].d;c.sd=sessions.map(function(s){return {d:'S'+s.n,r:byDate[s.d]||0};});c.rt=Object.keys(types).sort(function(a,b){return types[b]-types[a];}).map(function(k){return {n:k,v:types[k],c:T.gold};});c.ac=Object.keys(actions).sort(function(a,b){return actions[b]-actions[a];}).slice(0,6).map(function(k){return {n:k,v:actions[k],c:T.goldMid};});c.chars=Object.keys(chars).map(function(k){var x=chars[k],sum=x.faces.reduce(function(a,b){return a+b;},0);return {n:x.n,r:x.r,avg:x.faces.length?(sum/x.faces.length).toFixed(1):'—',tr:0};}).sort(function(a,b){return b.r-a.r;});}).catch(rcUnavailable(cfg.id));}
function rcHydrateAll(){return Promise.all(RC_LIVE.map(rcHydrate));}
/* Verification block: run from browser devtools after deploy.
   rcDie('[20]') === 20; rcDie('bad-json') === null; rcRows() returns every page;
   one campaign's outage must not blank the other (each rcHydrate catches its own). */
rcHydrateAll().finally(function(){ReactDOM.createRoot(document.getElementById('root')).render(h(App));});
